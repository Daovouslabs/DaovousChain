...
class SQLDatabaseSequentialChain(Chain):
    ...
    async def _acall(
        ...
        _lowercased_table_names = [name.lower() for name in _table_names]
        table_names_from_chain = await self.decider_chain.apredict_and_parse(**llm_inputs)
        table_names_to_use = [
            name
            for name in table_names_from_chain
            if name.lower() in _lowercased_table_names
        ...
        if not table_names_to_use:
            return {self.output_key: "Task fail, can not decide which tables to use."}
        new_inputs = {
            self.sql_chain.input_key: inputs[self.input_key],
            "table_names_to_use": table_names_to_use,
        }
